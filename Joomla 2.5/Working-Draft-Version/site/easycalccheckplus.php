...
class plgSystemEasyCalcCheckPlus extends JPlugin
{
    ...
    public function onAfterRender()
    {
        if($this->_load_ecc == true)
        {
            ...
            if(($this->params->get('type_calc') OR $this->params->get('recaptcha') OR $this->params->get('question')) AND !empty($match_extension))
            {
                ...
                $add_string = '<!-- EasyCalcCheck Plus - Kubik-Rubik Joomla! Extension --><div id="easycalccheckplus">';
    ...
    private function loadEcc($option, $task, $view, $func, $layout)
    {
        ...
        else
        {
            ...
            elseif($option == 'com_users')
            {
                if($layout != 'confirm' AND $layout != 'complete')
                {
                    ...
                    {
                        $this->_extension_info = array('com_users', '<form[^>]+task=user.login.+</form>', '<label id=".+"', '<button type="submit" class=".*">');
                    }
            ...
            elseif($option == 'com_easybookreloaded') // Easybook Reloaded - tested with version 2.5-1
            {
            ...
            elseif($option == 'com_phocaguestbook') // Phoca Guestbook - tested with version 2.0.5
            {
            ...
            elseif($option == 'com_comprofiler') // Community Builder - tested with version 1.8
            {
                if($task == 'registers')
                {
            ...
            elseif($option == 'com_flexicontact') // FlexiContact - tested with version 5.10
            {
            ...
            elseif($option == 'com_kunena') // Kunena Forum - tested with version 1.7.2
            {
            ...
            {
                $this->_extension_info = array('com_community', '<form[^>]+id="jomsForm".+</form>', '<label id=".+>', '<div[^>]+cwin-wait.*></div>');
            ...
            elseif($option == 'com_virtuemart') // Virtuemart - tested with version 2.0.2
            {
                $this->_extension_info = array('com_virtuemart', '<form[^>]+id="askform".+</form>', '<label>', '<input[^>]*type="submit" name="submit_ask"[^>]*/>');
        ...
        // Clear user_login session variable to avoid errors if a user login via the module
